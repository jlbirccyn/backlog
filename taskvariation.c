...
{
	FILE *fptr;
 	fptr = fopen("data3.dat","w");
 	int kmax=1, nbtasks=1,iteration=0;
 	bool val= false;
	double comb,prod;
	srand(time(NULL));  
 	while(val==false)
	...
	for(int i=0; i< nbtasks; i++){
   	 backlogs[i]=rand() % kmax +1;
	}
	prod=nbstateproduct(nbtasks,backlogs);
        if(comb==prod && iteration>2) val=true;
	fprintf(fptr,"%d ",nbtasks);
	fprintf(fptr,"%d ",kmax);
	fprintf(fptr,"%g ",prod);
	fprintf(fptr,"%g ",comb);
	fprintf(fptr,"\n");
	iteration=iteration+1;
	if(iteration==50) val=true;
	kmax=kmax+1;
	nbtasks=nbtasks+1;
	}
